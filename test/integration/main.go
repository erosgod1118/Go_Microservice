...
func startMetadaService(ctx context.Context, registry discovery.Registry) *grpc.Server {
	log.Println("Starting metadata service on " + metadataServiceAddr)

	h := metadatatest.NewTestMedataGRPCServer()
...
func main() {
	...
	metadataSrv := startMetadaService(ctx, registry)
